...
public class Ball: MonoBehaviour{
    ...
    float lefLimit, rightLimit;
    void Start(){

        lefLimit = Utils.GetLeftSide() - GetComponent<BoxCollider2D>().size.x / 2;
    ...
    private void Update (){
        if(transform.position.x < lefLimit || transform.position.x > rightLimit)
        {
            NewGame();
        }
    }
